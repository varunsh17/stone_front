...
import { ActivityDetails, AddBLDRequest, BLDDetails, LeaseDetails } from '../business-logic/admin-interfaces';
import { useCallback, useContext, useEffect, useState } from 'react';
import { ActionType, EditPageProps } from './lease-details-table';
import { addBuilding, deleteBuilding, getBuilding, updateBuilding } from '../business-logic/handle-building-modifications';
import { SessionContextValue, SessionStateContext } from '../../sessionContext';

const BLDEditPage: React.FC = () => {
  ...
  const handleEdit = useCallback(async (data: string) => {
    ...
    setEditKey(data);
    let response;
      response = await getBuilding(data, session);
    if (response) {
  ...
  const handleDelete = async (data: string) => {
    const response = await deleteBuilding(data, session);
    if (response) {
  ...
  const getReqData = (textfieldsVal: { [id: string]: string }) => {
    let data: AddBLDRequest = {
      building_no: '',
      address: ''
    };
    if (textfieldsVal) {
      data = {
        building_no: textfieldsVal['BUILDING NO.'],
        address: textfieldsVal['ADDRESS']
      };
    }
    return data;
  };
  ...
  const handleOnConfirm = async (action: ActionType, textfieldsVal: { [id: string]: string }) => {
    ...
    if (action === ActionType.Edit && textfieldsVal) {
      response  = await updateBuilding(getReqData(textfieldsVal), session);
    } else if (action === ActionType.Add && textfieldsVal) {
      response  = await addBuilding(getReqData(textfieldsVal), session);
    }
    if (response) {
      // show success modal
      if (action === ActionType.Add && textfieldsVal) {
        setTableEntry(tableEntry
          .filter(i => i !== editkey)
          .concat(textfieldsVal['BUILDING NO.']));
      }
  ...
  const handleOnAdd = async () => {
    setEditData({building_no: '', address: ''});
  ...
  return (
    <Box sx={{ backgroundColor: "#F5F5F5", minHeight: "100vh" }}>
            ...
            <Grid item xs={10} sx={{borderRadius: "6px", border:"1px solid #0000001A", boxShadow: "0 2px 8px rgba(0,0,0,0.05)",}}>
              <Grid container data-testid="editDetails" justifyContent="center" alignItems="center" sx={{ borderRadius: "6px 6px 0 0", backgroundColor: "#4CAF50", p: 2 }}>
                ...
                <Grid item xs={7} style={{ textAlign: 'justify' }}>
                  <Typography variant="h6" fontSize="18px" align="left" color="white" fontWeight="bold">EDIT BUILDING NUMBER</Typography>
                </Grid>
              </Grid>
      ...
      {<ReusableModal
        open={open}
        titleText={`${action === ActionType.Edit ? 'EDIT' : 'ADD'} BUILDING NUMBER`}
        headers={['BUILDING NO.','ADDRESS']}
        onClose={resetState}
